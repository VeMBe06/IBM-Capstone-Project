...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        ...
        filtered_df['Outcome'] = filtered_df['class'].map({0:1, 1:1})
        fig = px.pie(filtered_df, values='Outcome', 
                     names = 'class', 
                     title = f'Pie chart for site {entered_site}'
                     ) 
        return fig
...
def get_scatter_chart(entered_site, payload_range):
    ...
    if entered_site == 'ALL':
        ...
        fig.update_layout(xaxis_range=payload_range)
        fig.update_layout(title=f'Correlation between Payload and Success for site {entered_site}')
